...
const Demo5Layout = () => {
  ...
  // v2: per-path hide rules
  const hideConfig = [
    { path: '/ai-agent', hideSidebar: true, hideToolbar: true },
    // add more paths here when needed
  ];
  const matches = hideConfig.find((cfg) => pathname.startsWith(cfg.path));
  const hideSidebar = matches?.hideSidebar === true;
  const hideToolbar = matches?.hideToolbar === true;
    ...
    <>
      ...
      <div className="flex grow flex-col in-data-[sticky-header=on]:pt-(--header-height)">
        ...
        <div className="w-full flex px-0 lg:ps-4">
          ...
          <main className="flex flex-col grow">
            {!pathname.includes('/public-profile/') && !hideToolbar && (
              <Toolbar>
                <ToolbarHeading />
                <ToolbarActions>
                  <Popover>
