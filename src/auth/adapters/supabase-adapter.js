...
export const SupabaseAdapter = {
  ...
  async login(email, password) {
    ...
    // Demo authentication for demo@kt.com
    if (email === 'demo@kt.com' && password === 'demo123') {
      console.log('SupabaseAdapter: Demo login successful');
      return {
  ...
  async getCurrentUser() {
    // Check if we have demo auth tokens
    const auth = authHelper.getAuth();
    if (
      auth &&
      auth.access_token &&
      auth.access_token.startsWith('demo_access_token_')
    ) {
      ...
      return {
        ...
        company_name: 'Keen Themes',
        companyName: 'Keen Themes',
